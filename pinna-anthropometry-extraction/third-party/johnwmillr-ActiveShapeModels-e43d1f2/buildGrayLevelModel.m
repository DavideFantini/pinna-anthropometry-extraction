function grayModel = buildGrayLevelModel(images,shapeModel,varargin)
% BUILDGRAYLEVELMODEL
%
%	INPUT
...
keys = {'save_model','resolutions','region_size','view_model','filter_sigs'};
default_values = {0,6:-1:3,[],0,[]};
[save_model, downsample_factors, region_size, view_model, filter_sigs] = parseKeyValuePairs(varargin,keys,default_values);
if save_model
    fprintf('\n\n')
...
if isempty(filter_sigs)
    filter_sigs = linspace(0.8,0.35,n_resolutions);
end

if isempty(region_size)
    region_size = round(linspace(20,5,n_resolutions)); % Should go from about 20 to 5
end
% region_size = 15;% PARAMETER
...
for n_resolution = 1:n_resolutions
    ...
    C = 5; % Sigmoid equalization% PARAMETER
