function varargout = ASM(im_original,shapeModel,grayModel,varargin)
...
keys = {'save_video','save_gif','visualize','facefinder','dist_metric','evolutions','vis_grid','search_sizes','n_pcs','m','pinna_parts_idx'};
default_values = {0,0,0,'auto','pca',4,0,round(linspace(8,3,n_resolutions)),10,1,{}};
[save_video, save_gif, vis, face_find_method, dist_metric, evolutions, vis_grid, search_sizes,n_pcs,m,pinna_parts_idx] =...
...
search_sizes = round(linspace(search_sizes(end),search_sizes(1),n_resolutions));
...
%             [x_original_estimate, h_im] = estimateFaceLocation(im,x_mean,GM.Info.SmoothingFilter,vis);
            x_original_estimate = x_mean;
            if vis
                h_im = figure('WindowState','maximize');
            end
        else
            [x_original_estimate, h_im] = placeShape(im_original,x_mean);
        end
%         tic
        drawnow
        ...
        if vis, figure(h_im), hold off
            imagesc(im); colormap(jet); axis xy; axis equal; axis tight;plotLandmarks(x_aligned,'hold',1,'pinna_parts_idx',pinna_parts_idx), end
    end
    
    % Evolve estimate of face location, adjusting landmarks w/in model space
    if isscalar(evolutions)
       n_evolutions = evolutions;
    else
        n_evolutions = evolutions(n_resolution);
    end
...
%                     im_region = imcrop(im,[iPixel(1)-rc/2 iPixel(2)-rc/2 rc rc]);
                    im_region = im(round(max(iPixel(2)-rc/2,1):min(iPixel(2)-rc/2+rc,size(im,1))),...
                        round(max(iPixel(1)-rc/2,1):min(iPixel(1)-rc/2+rc,size(im,2))));
...
%         b=b_suggested;
        ...
        if vis % View the current evolution            
            imagesc(im); colormap(jet); axis xy; axis equal; axis tight; plotLandmarks(x_current,'hold',1,'linewidth',3,'pinna_parts_idx',pinna_parts_idx), hold on
...
if vis
    figure(gcf), hold off, imagesc(im_original); colormap(jet); axis xy; axis equal; axis tight;
%     h_orig  = plotLandmarks(x_original_estimate,'hold',1,'linestyle','--','point_color','r');
    h_final = plotLandmarks(x_final,'hold',1,'color','#40444A','linewidth',1,'point_color','k','linestyle','-','pinna_parts_idx',pinna_parts_idx);
...
x_final=[x_final(1:2:end),x_final(2:2:end)];
